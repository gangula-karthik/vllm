...
class ShukaAudioFeatureInputs(TensorSchema):
    """
    Dimensions:
    - b: batch size
    - n: number of chunks
...
class ShukaAudioEmbeddingInputs(TensorSchema):
    """
    Dimensions:
    - b: batch size
    - na: number of audios
...
ShukaAudioInputs: TypeAlias = ShukaAudioFeatureInputs | ShukaAudioEmbeddingInputs


class ShukaProcessingInfo(BaseProcessingInfo):
    def get_hf_processor(self, **kwargs: object) -> ProcessorMixin:
        config = self.ctx.model_config.hf_config

        # NOTE: ShukaProcessor is not registered in the repo's `auto_map`
        # (only AutoConfig/AutoModel are), so AutoProcessor.from_pretrained
        # can't discover it. Load it directly instead, same as ShukaPipeline
        # (shuka_pipeline.py) does.
        from transformers.dynamic_module_utils import get_class_from_dynamic_module

        model_config = self.ctx.model_config
        shuka_processor_cls = get_class_from_dynamic_module(
            "shuka_processing.ShukaProcessor",
            model_config.model,
            revision=model_config.revision,
        )

        # ShukaProcessor forwards audio straight to the audio_processor and
        # expects Whisper-style `input_features` back, so a bare
        # WhisperFeatureExtractor is what it needs (not a full
        # WhisperProcessor, which also bundles a tokenizer).
        audio_config = config.audio_config
        audio_model_id = config.audio_model_id or audio_config._name_or_path
        audio_processor = WhisperFeatureExtractor.from_pretrained(audio_model_id)

        hf_processor = shuka_processor_cls(
            audio_processor=audio_processor,
            tokenizer=self.get_tokenizer(),
            stack_factor=config.stack_factor,
        )

        # NOTE: Shuka processing definition uses '<|eot_id|>' as the
    ...
    def get_target_channels(self) -> int:
        """Return target audio channels for Shuka models (mono)."""
        return 1
...
class ShukaDummyInputsBuilder(BaseDummyInputsBuilder[ShukaProcessingInfo]):
...
class ShukaMultiModalProcessor(BaseMultiModalProcessor[ShukaProcessingInfo]):
    def _call_hf_processor(
        ...
        # NOTE: ShukaProcessor.__call__ only accepts a single `audio=` array
        # with no chunking or `audio_lens`/`audio_num_chunks` output, so
        # compute chunked Whisper features and token lengths directly here
        # to match the schema `_get_mm_fields_config` expects.
        config = self.info.ctx.model_config.hf_config
        feature_extractor = self.info.get_feature_extractor(**mm_kwargs)
        sampling_rate = feature_extractor.sampling_rate
        chunk_samples = int(feature_extractor.chunk_length * sampling_rate)
        stack_factor = config.stack_factor
        ...
        for audio in audios:
            ...
            num_chunks = max(1, -(-len(audio) // chunk_samples))  # ceil div
            ...
            for i in range(num_chunks):
                ...
                audio_lens_list.append(chunk_len)

                # WhisperFeatureExtractor pads every chunk to a fixed
                # `chunk_length`, so the encoder output size only depends on
                # the padded mel frame count, not the real audio length
                # (`audio_lens`, used only for the encoder's attention mask).
                # See WhisperEncoder._get_feat_extract_output_lengths.
                mel_frames = features.shape[-1]
                nb_encoder_frames = (mel_frames - 1) // 2 + 1
                audio_token_len_list.append(
                    -(-nb_encoder_frames // stack_factor)  # ceil div
                )

        output = BatchFeature(
            dict(
                input_ids=[
                    self.info.get_tokenizer().encode(prompt, add_special_tokens=False)
                ],
                audio_features=torch.stack(audio_features_list),
                audio_lens=torch.tensor(audio_lens_list, dtype=torch.int32),
                audio_token_len=torch.tensor(audio_token_len_list, dtype=torch.int32),
                audio_num_chunks=torch.tensor(num_chunks_list, dtype=torch.int32),
            ),
            tensor_type="pt",
        )
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_shuka(item_idx: int):
        ...
        return [
            PromptReplacement(
                modality="audio",
                target="<|audio|>",
                replacement=get_replacement_shuka,
            )
        ]
...
class ShukaProjector(nn.Module):
    def __init__(self, config: ShukaConfig):
        super().__init__()
...
@MULTIMODAL_REGISTRY.register_processor(
    ShukaMultiModalProcessor,
    info=ShukaProcessingInfo,
    dummy_inputs=ShukaDummyInputsBuilder,
)
class ShukaModel(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        config: ShukaConfig = vllm_config.model_config.hf_config
        ...
        with self._mark_tower_model(vllm_config, "audio"):
            ...
            self.multi_modal_projector = ShukaProjector(config)
    ...
    def _parse_and_validate_audio_input(
        ...
        if audio_features is not None:
            return ShukaAudioFeatureInputs(
                type="audio_features",
                data=audio_features,
                lens=audio_lens,
        ...
        if audio_embeds is not None:
            return ShukaAudioEmbeddingInputs(type="audio_embeds", data=audio_embeds)

        raise AssertionError("This line should be unreachable.")

    def _process_audio_input(
        self,
        audio_input: ShukaAudioInputs,
